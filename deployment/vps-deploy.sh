...
if [ "$SWARM_ENABLED" = "1" ] || { [ "$SWARM_ENABLED" = "auto" ] && [ "$IS_SWARM_MANAGER" = "1" ]; }; then
  ...
  if docker compose ps --services 2>/dev/null | grep -q '^zapmass$'; then
    echo "==> (compose) atualizar serviço zapmass (sem force-recreate — evita queda em massa dos chips)"
    docker compose up -d --no-deps --build zapmass
    echo "==> aguardando 20s para API estabilizar (janela de graça pós-deploy)..."
    sleep 20
  fi
...
if [ -d /opt/zapmass/clientes ] && ls /opt/zapmass/clientes/*/docker-compose.yml >/dev/null 2>&1; then
  ...
  for dir in /opt/zapmass/clientes/*/; do
    ...
    if [ -f "${dir}.deploy-skip" ]; then
      echo "    - cliente: ${slug} (skip — .deploy-skip)"
      continue
    fi
    if [ -n "${DEPLOY_SKIP_CLIENTS:-}" ]; then
      _skip_slug=0
      IFS=',' read -r -a _dep_skip <<< "${DEPLOY_SKIP_CLIENTS}"
      for _ds in "${_dep_skip[@]}"; do
        _ds="$(echo "$_ds" | tr -d '[:space:]')"
        if [ -n "$_ds" ] && [ "$slug" = "$_ds" ]; then
          _skip_slug=1
          break
        fi
      done
      unset _dep_skip _ds
      if [ "${_skip_slug}" = "1" ]; then
        echo "    - cliente: ${slug} (skip — DEPLOY_SKIP_CLIENTS)"
        continue
      fi
      unset _skip_slug
    fi
...
if [ "${PRUNE_BUILDER_AFTER_DEPLOY:-0}" = "1" ]; then
  echo "==> docker builder prune (PRUNE_BUILDER_AFTER_DEPLOY=1)"
...
if [ "${PRUNE_AFTER_DEPLOY:-0}" = "1" ]; then
  ...
  if [ "${PRUNE_BUILDER_AFTER_DEPLOY:-0}" != "1" ]; then
    echo "==> prune desativado (cache de build preservado; use PRUNE_BUILDER_AFTER_DEPLOY=1 no .env)"
  fi
fi
