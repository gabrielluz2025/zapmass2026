...
# Repassa VITE_*, MERCADOPAGO_*, INFINITEPAY_*, ZAPMASS_*, TRUST_PROXY, WWEBJS_WEB_VERSION_URL… (ver .env.example)
if [ -f .env ]; then
  while IFS='=' read -r k v; do
    k="${k//$'\r'/}"
    v="${v//$'\r'/}"
    case "$k" in
      VITE_*|MERCADOPAGO_*|INFINITEPAY_*|ZAPMASS_*|HOST_PORT|METRICS_TOKEN|WA_WORKER_REPLICAS|ENSURE_SWAP_ON_DEPLOY|BUILDKIT_MAX_PARALLELISM|SWAP_SIZE_MB|SUBSCRIPTION_ENFORCE|ADMIN_EMAILS|FIREBASE_WEB_API_KEY|MAX_STAFF_PASSWORD_ACCOUNTS|WWEBJS_WEB_VERSION_URL|TRUST_PROXY|TRUST_PROXY_HOPS|EVOLUTION_WEBHOOK_TOKEN|RESEND_API_KEY|EMAIL_FROM|EMAIL_REPLY_TO|SUGGESTION_NOTIFY_EMAIL|PUBLIC_APP_URL)
        export "$k=$v"
        ;;
    esac
  # Aceitar também `export VAR=...` (sem isso o grep não apanha a linha e o Swarm fica sem MERCADOPAGO/FIREBASE/etc.).
  done < <(grep -E '^[[:space:]]*(export[[:space:]]+)?(VITE_[A-Z0-9_]*|MERCADOPAGO_[A-Z0-9_]*|INFINITEPAY_[A-Z0-9_]*|ZAPMASS_[A-Z0-9_]*|HOST_PORT|METRICS_TOKEN|WA_WORKER_REPLICAS|ENSURE_SWAP_ON_DEPLOY|BUILDKIT_MAX_PARALLELISM|SWAP_SIZE_MB|SUBSCRIPTION_ENFORCE|ADMIN_EMAILS|FIREBASE_WEB_API_KEY|MAX_STAFF_PASSWORD_ACCOUNTS|WWEBJS_WEB_VERSION_URL|TRUST_PROXY|TRUST_PROXY_HOPS|EVOLUTION_WEBHOOK_TOKEN|RESEND_API_KEY|EMAIL_FROM|EMAIL_REPLY_TO|SUGGESTION_NOTIFY_EMAIL|PUBLIC_APP_URL)=' .env | sed -E 's/^[[:space:]]*export[[:space:]]+//' || true)
