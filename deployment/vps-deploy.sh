...
if [ -f .env ]; then
  while IFS='=' read -r k v; do
    ...
    v="${v//$'\r'/}"
    case "$k" in
      VITE_*|MERCADOPAGO_*|ZAPMASS_*|HOST_PORT|METRICS_TOKEN|WA_WORKER_REPLICAS|ENSURE_SWAP_ON_DEPLOY|BUILDKIT_MAX_PARALLELISM|SWAP_SIZE_MB|SUBSCRIPTION_ENFORCE|ADMIN_EMAILS|FIREBASE_WEB_API_KEY|MAX_STAFF_PASSWORD_ACCOUNTS|WWEBJS_WEB_VERSION_URL|TRUST_PROXY|TRUST_PROXY_HOPS|EVOLUTION_WEBHOOK_TOKEN|RESEND_API_KEY|EMAIL_FROM|EMAIL_REPLY_TO|SUGGESTION_NOTIFY_EMAIL|NEW_CLIENT_NOTIFY_EMAIL|PUBLIC_APP_URL|ALLOWED_ORIGINS)
        export "$k=$v"
        ;;
    esac
