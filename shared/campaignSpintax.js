...
export function extractCampaignSpintaxBlocks(text) {
    const blocks = [];
...
export function buildCampaignSpintax(options) {
...
export function resolveCampaignSpintax(text, rotationIndex) {
    const safeIndex = Number.isFinite(rotationIndex) ? Math.floor(rotationIndex) : 0;
    return text.replace(SPINTAX_BLOCK_RE, (_, inner) => {
...
export function countCampaignSpintaxVariations(text) {
...
export function analyzeCampaignSpintax(text, rotationIndex = 0) {
...
export function campaignRotationIndexFromPhone(phone) {
 ...
 */
export function hasUnresolvedCampaignTemplateTokens(text) {
 ...
 */
export function sanitizeCampaignTemplateForOutbound(text, rotationIndex = 0) {
...
export function isCampaignSpintaxBlock(value) {
