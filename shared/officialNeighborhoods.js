...
const STATIC_REGISTRY = [
    {
        ...
        aliases: Object.fromEntries(Object.entries(BLUMENAU_NB_ALIASES).map(([k, v]) => [k, v])),
    },
    {
        city: 'Indaial',
        state: 'SC',
...
export function matchCityOfficialNeighborhood(city, state, raw) {
    const entry = findRegistryEntry(city, state);
    if (!entry)
        return null;
...
export function matchOfficialNeighborhoodInList(raw, officialList) {
    const key = normNbKey(raw);
    if (!key)
        return null;
 ...
 */
export function resolveContactNeighborhoodForCity(city, state, raw, officialList) {
...
export function matchOfficialNeighborhoodForBlumenau(raw) {
    return matchCityOfficialNeighborhood('Blumenau', 'SC', raw);
}
export function officialSpreadCoord(city, state, index, total) {
