...
export function isFullInboxSyncEnabled() {
    return !['0', 'false', 'no', 'off'].includes(String(process.env.WA_FULL_INBOX_SYNC ?? '1').trim().toLowerCase());
}
/** Baileys/Evolution: baixar histórico completo do celular na conexão (settings.syncFullHistory). */
export function isEvolutionFullHistorySyncEnabled() {
...
export function evolutionSyncSparseConvLimit() {
...
export function socketInboxMsgTailLimit() {
