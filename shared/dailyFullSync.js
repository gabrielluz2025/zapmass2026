...
export function fullSyncIntervalMs() {
    const hours = Number(process.env.WA_FULL_SYNC_COOLDOWN_HOURS ?? 24);
    if (!Number.isFinite(hours) || hours < 1)
        return DEFAULT_FULL_SYNC_INTERVAL_MS;
    return Math.floor(hours * 60 * 60 * 1000);
}
export function isFullSyncDue(lastAtMs, now = Date.now()) {
    if (!lastAtMs || lastAtMs <= 0)
        return true;
 ...
 */
export function isFullSyncDueAfterRestart(lastAtMs, processStartedAt, now = Date.now()) {
    if (processStartedAt > 0 && lastAtMs > 0 && lastAtMs < processStartedAt)
        return true;
