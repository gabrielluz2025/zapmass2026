...
export function formatTodayYmdInZone(timeZone) {
...
function formatZonedYmdHmKey(utcMs, timeZone) {
    const fmt = new Intl.DateTimeFormat('en-CA', {
        timeZone,
        year: 'numeric',
        month: '2-digit',
...
export function dayOfWeekForCalendarDateInZone(dateStr, timeZone) {
...
export function ymdInZoneFromUtcInstant(isoUtc, timeZone) {
    if (!isoUtc)
        return null;
...
function zonedWallClock(utcMs, timeZone) {
 ...
 */
export function computeNextRunIso(slots, timeZone, afterMs) {
    if (!slots.length || !timeZone)
        return null;
...
export function formatScheduleSlotLine(slot) {
    const d = DAY_SHORT_PT[slot.dayOfWeek] ?? `D${slot.dayOfWeek}`;
    return `${d} ${slot.time}`;
}
export function buildWeekAgendaText(slots, timeZone) {
    if (!slots.length)
        return 'Sem horários';
