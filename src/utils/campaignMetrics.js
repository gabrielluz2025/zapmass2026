...
import { isConversationalMultiStepCampaign, resolveCampaignEffectiveStageCount } from './campaignStageCount';
/** Planejamento de envios: contactos × etapas (metadata ou inferido dos contadores). */
export function getCampaignPlannedSendTotal(campaign) {
 ...
 */
export function getCampaignDeliverySuccessRatePct(campaign) {
...
export function getCampaignProgressMetrics(campaign) {
    ...
    let effectiveProcessed = plannedSendTotal <= 0 ? 0 : Math.min(plannedSendTotal, Math.max(reported, ok + fail));
    if (isConversationalMultiStepCampaign(campaign) &&
    ...
    const progressPct = plannedSendTotal > 0 ? Math.min(100, Math.round((effectiveProcessed / plannedSendTotal) * 100)) : 0;
    const successRatePct = effectiveProcessed > 0 ? Math.min(100, Math.round((ok / effectiveProcessed) * 100)) : 0;
    return {
        total,
        plannedSendTotal,
        ok,
...
export function isRunningStatusButWorkComplete(c) {
    if (c.status !== CampaignStatus.RUNNING && c.status !== CampaignStatus.DRAFT)
        return false;
 ...
 */
export function healCampaignCounters(c) {
    const m = getCampaignProgressMetrics(c);
    if (m.ok === (c.successCount ?? 0) &&
...
export function healStuckCampaignStatus(c) {
    ...
    if (counters.status === CampaignStatus.DRAFT &&
...
export function healStuckRunningCampaignsList(list) {
 ...
 */
export function mergeCampaignMetricsWithReport(base, report) {
    ...
    if (processedFromReport <= 0 && failedCount <= 0)
        return base;
