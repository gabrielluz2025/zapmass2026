export function mergeChatMessageLists(a, b) {
    const byId = new Map();
    for (const m of a) {
        if (m?.id)
            byId.set(m.id, m);
 ...
 */
export function ensureLatestPreviewInMessages(conv) {
