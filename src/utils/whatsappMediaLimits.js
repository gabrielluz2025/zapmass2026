...
const MB = 1024 * 1024;
const toMbLabel = (bytes) => `${(bytes / MB).toFixed(1)} MB`;
export const classifyWhatsAppAttachmentKind = (mimeType) => {
 ...
 */
export function mediaShouldSendAsDocument(file) {
    const kind = classifyWhatsAppAttachmentKind(file.type);
    if (kind === 'video')
        return file.size > WHATSAPP_VIDEO_MAX_BYTES;
    if (kind === 'image')
        return file.size > WHATSAPP_IMAGE_MAX_BYTES;
    if (kind === 'audio')
        return file.size > WHATSAPP_AUDIO_MAX_BYTES;
    return true;
}
/** Compat legado: mantém o comportamento antigo usado no chat. */
export function videoShouldSendAsDocument(file) {
    return classifyWhatsAppAttachmentKind(file.type) === 'video' && file.size > WHATSAPP_VIDEO_MAX_BYTES;
}
export const explainWhatsAppMediaFallback = (file) => {
    const kind = classifyWhatsAppAttachmentKind(file.type);
