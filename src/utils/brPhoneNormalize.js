 ...
 */
export function canonicalBrazilMobileKey(digits) {
    ...
    if (d.length === 13) {
        ...
        if (afterDdd.length === 9 && afterDdd[0] === '9') {
            ...
            if (BR_LANDLINE_SUB_PREFIX.test(rest))
                return `55${ddd}${rest}`;
            return d;
        }
        return d;
    ...
    if (d.length === 12) {
        ...
        if (local.length === 8 && BR_MOBILE_SUB_PREFIX.test(local))
 ...
 */
export function stripBrazilNationalTrunkZero(digits) {
 ...
 */
export function repairCorruptedBrJidDigits(digits) {
    const d = phoneDigitsOnly(digits);
    if (!d)
        return d;
    if (d.length === 12 && d.startsWith('547'))
        return `5547${d.slice(3)}`;
    if (d.length === 11 && d.startsWith('547'))
        return `479${d.slice(3)}`;
    return d;
}
/** Chave única por telefone (BR: 0 tronco + DDI 55 quando faltar). */
export function normPhoneKey(p) {
