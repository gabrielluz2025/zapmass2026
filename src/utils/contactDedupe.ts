...
function pickFilled(keeper: string | undefined, others: Array<string | undefined>): string | undefined {
  if (filled(keeper)) return keeper;
  for (const o of others) {
    if (filled(o)) return o;
  }
  return keeper;
}

/** Pontua o cadastro mais completo para permanecer como linha única na base. */
export function contactDedupeRichness(c: Contact): number {
  let score = 0;
  const name = String(c.name || '').trim();
  if (name && !isSuspiciousContactName(name)) score += 120;
  else score += Math.min(name.length, 15);
  if (c.status === 'VALID') score += 12;
  if (filled(c.city)) score += 6;
  if (filled(c.street)) score += 6;
  if (filled(c.neighborhood)) score += 3;
  if (filled(c.zipCode)) score += 3;
  if (filled(c.birthday)) score += 8;
  if (filled(c.email)) score += 4;
  if (filled(c.notes)) score += 3;
  if (filled(c.followUpAt)) score += 4;
  if (filled(c.profilePicUrl)) score += 6;
  if (c.religiousMemberProfile) score += 8;
  score += Math.min((c.tags || []).length, 10);
  score += Math.min(Number(c.campaignMessagesReceived) || 0, 20);
  return score;
}

export function pickDuplicateKeeper(group: Contact[]): Contact {
  if (group.length === 0) throw new Error('Grupo de duplicados vazio.');
...
function mergeNotes(keeper: string | undefined, others: Array<string | undefined>): string | undefined {
  const parts: string[] = [];
  ...
  if (parts.length === 0) return keeper;
  return parts.join('\n');
}
...
export function mergeDuplicateGroup(
...
): { keeper: Contact; extraIds: string[]; updates: Partial<Contact> } {
  ...
  const name =
    !isSuspiciousContactName(keeper.name || '') || !betterName ? keeper.name : betterName;

  const tags = [...new Set([...(keeper.tags || []), ...others.flatMap((c) => c.tags || [])])].filter(
    Boolean
  );
  ...
  const campaignMessagesReceived = Math.max(
    0,
    ...group.map((c) => Number(c.campaignMessagesReceived) || 0)
  );

  const followUps = group.map((c) => c.followUpAt).filter(filled) as string[];
  ...
  const optional: Array<[keyof Contact, string | undefined]> = [
  ...
  if (religious) updates.religiousMemberProfile = religious;

  if (group.some((c) => c.marketingOptOut)) updates.marketingOptOut = true;
  if (group.some((c) => c.marketingOptIn)) updates.marketingOptIn = true;
  if (campaignMessagesReceived > 0) updates.campaignMessagesReceived = campaignMessagesReceived;

  return { keeper, extraIds, updates };
}

/** Troca IDs extras pelo keeper e remove repetição na mesma lista. */
export function remapListContactIds(
  ids: string[],
  ...
  for (const raw of ids) {
    ...
    if (mapped !== String(raw)) changed = true;
    if (seen.has(mapped)) {
      changed = true;
      continue;
    }
