...
import { fuzzyResolveCityWithIbge } from './ibgeCityLookup';
...
export function normPlaceKey(s) {
    return normKeyPart(s);
}
/** Chave de bairro tolerante a letras duplicadas (ex.: Fortaaleza → Fortaleza). */
export function normNeighborhoodKey(s) {
    return normPlaceKey(s).replace(/(.)\1+/g, '$1');
}
function collapseDoubledLetters(s) {
    return s.replace(/([\p{L}])\1+/giu, '$1');
}
/** Escolhe o nome mais correto quando dois bairros são a mesma chave canônica. */
export function pickCanonicalNeighborhoodName(a, b) {
    if (normNeighborhoodKey(a) !== normNeighborhoodKey(b))
        return a;
    if (a.length !== b.length)
        return a.length < b.length ? a : b;
    if (/(.)\1/.test(a) && !/(.)\1/.test(b))
        return b;
    if (/(.)\1/.test(b) && !/(.)\1/.test(a))
        return a;
    return a;
}
...
export function stripBrokenEncoding(raw) {
    return cleanWhitespace(String(raw || '')
        .replace(/\uFFFD/g, '')
        .replace(/[\u0000-\u0008\u000b\u000c\u000e-\u001f]/g, ''));
}
/** Corrige texto UTF-8 lido como Latin-1 (ex.: Agrolndia → Agrolândia). */
export function repairUtf8Mojibake(raw) {
...
function capitalizeHyphenated(lowerWord) {
    return lowerWord
        .split('-')
        .map((seg) => {
        if (!seg)
...
export function titleCasePlaceName(raw) {
    ...
    return s
        ...
        if (i > 0 && PT_PARTICLES_LOWER.has(lower))
            return lower;
        return capitalizeHyphenated(lower);
    })
        .join(' ');
}
export function normalizeContactState(raw) {
...
export function normalizeContactNeighborhood(raw, cityHint) {
...
export function normalizeContactZipCode(raw) {
...
export function parseEmbeddedCityState(cityRaw) {
...
export function parseGeoFilterCity(raw) {
...
export function citiesMatch(contactCity, filterCityLabel, contactState) {
    ...
    if (fc.state && contactState) {
        ...
        if (st && normPlaceKey(st) !== normPlaceKey(fc.state))
            return false;
    }
    return true;
}
export function knownUfForCity(city) {
 ...
 */
export function resolveAddressCityState(input, ibgeIndex) {
...
export function resolveContactCityState(input, ibgeIndex) {
    const fromAddress = resolveAddressCityState({ city: input.city, state: input.state }, ibgeIndex);
    if (fromAddress.city)
        return fromAddress;
...
function tryResolveMunicipality(name, stateHint, ibgeIndex) {
    ...
    if (uf)
        return { city: titleCasePlaceName(city), state: uf };
    return null;
}
/**
...
function inferUfSignals(stateHint, phone, zipCode) {
    const out = [];
    if (stateHint)
        out.push(stateHint);
 ...
 */
export function gazetteerKeyFromCityField(cityRaw) {
    const s = repairUtf8Mojibake(cityRaw || '');
    if (!s)
        return null;
    const direct = normNeighborhoodKey(s);
    if (NEIGHBORHOOD_GAZETTEER[direct])
        return direct;
    ...
    for (const cityKey of Object.keys(KNOWN_CITY_UF)) {
        if (nbKey.length > cityKey.length + 2 && nbKey.endsWith(cityKey)) {
            ...
            if (NEIGHBORHOOD_GAZETTEER[prefix])
                return prefix;
        }
    }
...
function parseNeighborhoodCityCompound(cityRaw) {
    ...
    if (!part1 || !part2 || part2.length < 3)
        return null;
    const part2Uf = normalizeContactState(part2);
    if (part2Uf && part2.length <= 3)
        return null;
    const cityUf = knownUfForCity(part2);
    const gazNb = gazetteerKeyFromCityField(part1) || normNeighborhoodKey(part1);
    if (!cityUf && !KNOWN_CITY_UF[normKeyPart(part2)])
        return null;
    if (!NEIGHBORHOOD_GAZETTEER[gazNb] && !KNOWN_CITY_UF[normKeyPart(part2)])
        return null;
    return {
        neighborhood: part1,
...
function resolveGazetteerNeighborhoodKey(cityRaw, nbRaw) {
    return (gazetteerKeyFromCityField(cityRaw) ||
...
function knownNeighborhoodMunicipality(nbKey, stateHint, phone, zipCode) {
    if (!nbKey)
        return null;
    ...
    for (const uf of ufSignals) {
        ...
        if (match)
            return { city: match.city, state: match.uf };
    }
    ...
    if (SC_VALE_ITAJAI_NB_KEYS.has(nbKey)) {
        ...
        if (sc)
            return { city: sc.city, state: sc.uf };
    }
    return null;
}
/** CEP → município (prefixos mais usados no Vale do Itajaí / SC). */
function municipalityFromCepPrefix(cep, stateHint) {
    ...
    if (!hit)
        return null;
    if (stateHint && stateHint !== hit.state)
        return null;
    return hit;
}
/**
 ...
 */
export function resolveGeoPlaceForContact(input, ibgeIndex, nbToCityMap) {
    ...
    const gazHit = knownNeighborhoodMunicipality(nbKey, stateHint, input.phone, input.zipCode);
    if (gazHit && (cityForResolve || nbRaw)) {
        ...
        const cityFieldKey = gazetteerKeyFromCityField(cityForResolve) ||
            normNeighborhoodKey(parseEmbeddedCityState(cityForResolve).city || cityForResolve);
        const nbSource = nbRaw && nbFieldKey === nbKey
            ? nbRaw
            : cityFieldKey === nbKey
                ? cityForResolve
 ...
 */
export function canonicalizeClusterCity(city, stateHint = '', phone, zipCode, ibgeIndex) {
    ...
    if (knownUf && parsed.city)
        return titleCasePlaceName(parsed.city);
    return titleCasePlaceName(parsed.city || city);
}
/** Aprende bairro → cidade a partir de contatos com município válido na base. */
export function buildNeighborhoodToCityMap(contacts, ibgeIndex) {
    const votes = new Map();
    const meta = new Map();
    const addVote = (nbKey, city, state) => {
        if (!nbKey || !city)
            return;
        const ck = `${normPlaceKey(city)}|${state}`;
        meta.set(ck, { city, state });
        const bucket = votes.get(nbKey) || new Map();
    ...
    const muniMemo = new Map();
    const resolveMuni = (name, stateHint) => {
    ...
    for (const c of contacts) {
        ...
        const embedded = parseEmbeddedCityState(cityRaw);
        const nbKey = gazetteerKeyFromCityField(embedded.city || cityRaw) ||
            normNeighborhoodKey(embedded.city || cityRaw);
        if (!NEIGHBORHOOD_GAZETTEER[nbKey])
            continue;
        for (const uf of inferUfSignals(stateHint || embedded.state, c.phone, c.zipCode)) {
            const hit = NEIGHBORHOOD_GAZETTEER[nbKey].find((x) => x.uf === uf);
            if (hit)
                addVote(nbKey, hit.city, hit.uf);
        }
    }
    ...
    for (const [nbKey, bucket] of votes) {
        ...
        if (hit && bestN >= 1)
            result.set(nbKey, hit);
    }
    // Fallback regional: bairro do Vale do Itajaí sem nenhum voto (cadastro sem UF/DDD/CEP).
    const nbOnlyCounts = new Map();
    for (const c of contacts) {
        const cityRaw = repairUtf8Mojibake(c.city || '');
        const embedded = parseEmbeddedCityState(cityRaw);
        const nbKey = gazetteerKeyFromCityField(embedded.city || cityRaw) ||
            normNeighborhoodKey(embedded.city || cityRaw);
        if (!SC_VALE_ITAJAI_NB_KEYS.has(nbKey) || result.has(nbKey))
            continue;
    ...
    for (const [nbKey, n] of nbOnlyCounts) {
        ...
        if (sc)
            result.set(nbKey, { city: sc.city, state: sc.uf });
    }
    return result;
}
const STREET_PREFIX_RE = /^(rua|r\.|avenida|av\.|av |travessa|trav\.|estrada|rodovia|praça|pça\.|alameda|al\.)\s/i;
/** Corrige CEP, rua ou bairro digitados no campo errado. */
export function repairMisplacedAddressFields(input) {
...
function expandStreetAbbreviation(raw) {
...
export function normalizeContactAddressFields(input, ibgeIndex, nbToCityMap) {
    ...
    if (street)
        out.street = titleCasePlaceName(street);
...
export function applyAddressNormalizationToContact(contact, ibgeIndex) {
    const norm = normalizeContactAddressFields(contact, ibgeIndex);
    return { ...contact, ...norm };
}
export function contactAddressChanged(before, after) {
    const fields = [
        'city',
        'state',
        'neighborhood',
